...
dataset = pd.read_csv('multistep_feature.csv',header=None)      #读取数据
...
x_test = data_output[:trainlen].reshape(trainlen,6)    #训练输出
y_train = data_input[trainlen:].reshape(testlen,-1)   #测试输入
y_test = data_output[trainlen:].reshape(testlen,6)    #测试输出
...
#loss = model.fit(x_train, x_test, epochs=50, batch_size=64,validation_data=(y_train, y_test), verbose=2, shuffle=False)
...
train_dnn = min_max_scaler_output.inverse_transform(train_dnn.reshape(-1,6))
train_dnn = train_dnn.reshape(-1,1)
train_output = min_max_scaler_output.inverse_transform(x_test.reshape(-1,6))
train_output = train_output.reshape(-1,1)

test_dnn = model.predict(y_train)
test_dnn = min_max_scaler_output.inverse_transform(test_dnn.reshape(-1,6))
test_dnn= test_dnn.reshape(-1,1)
test_output = min_max_scaler_output.inverse_transform(y_test.reshape(-1,6))
